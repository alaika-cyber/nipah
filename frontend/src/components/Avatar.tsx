...
export default function Avatar({ isSpeaking, isThinking, mouthOpen }: AvatarProps) {
  return (
    <div className="flex flex-col items-center w-full">
      <div
        className={`w-full max-w-md h-72 rounded-3xl border border-indigo-400/20 bg-gradient-to-b from-slate-900 to-slate-950 shadow-2xl overflow-hidden ${
      ...
      >
        <Canvas camera={{ position: [0, 0.3, 3.1], fov: 35 }}>
          ...
          <OrbitControls enablePan={false} enableZoom={false} maxPolarAngle={2.1} minPolarAngle={1.1} />
        </Canvas>
      </div>
      ...
      {isSpeaking && (
        <div className="flex items-end gap-1 mt-3 h-7">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="voice-bar w-1.5 bg-indigo-400 rounded-full" style={{ height: '8px' }} />
          ))}
        </div>
      )}

      <p className="mt-2 text-sm text-gray-400">
        {isSpeaking ? 'Dr. NiVa is speaking...' : isThinking ? 'Dr. NiVa is thinking...' : 'Dr. NiVa - Virtual Infectious Disease Consultant'}
      </p>
    </div>
  );
}
